...
fn build_scrcpy_args(config: &ScrcpyConfig, video_dir_fallback: Option<String>) -> Vec<String> {
    ...
    if config.session_mode == "mirror" && otg_enabled && otg_pure {
        ...
        if config.session_mode == "mirror" && config.crop_enabled.unwrap_or(false) {
            if let Some(preset) = &config.crop_preset {
                let crop = match preset.as_str() {
                    "16:9" => Some("16:9"),
                    "4:3" => Some("4:3"),
                    "1:1" => Some("1:1"),
                    _ => None,
                ...
                if let Some(crop_ratio) = crop {
                    args.push("--crop".to_string());
                    args.push(crop_ratio.to_string());
                }
            }
        }
...
mod tests {
    ...
    #[test]
    fn test_build_scrcpy_args_mirror_crop_enabled() {
        let config = ScrcpyConfig {
            device: "device1".to_string(),
            session_mode: "mirror".to_string(),
            bitrate: None,
            ...
            rotation: None,
            res: None,
            crop_enabled: Some(true),
            crop_preset: Some("16:9".to_string()),
        };

        let args = build_scrcpy_args(&config, None);
        assert!(args.contains(&"--crop".to_string()));
        assert!(args.contains(&"16:9".to_string()));
    }
}
